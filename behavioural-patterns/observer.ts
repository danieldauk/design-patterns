...
class Subscriber implements SubscriberInterface {
  ...
  update(issue: string) {
    console.log(`${this.name} latest magazine issue: ${issue}`);
  }
}
...
const clientCode = () => {
  ...
  // Alan latest magazine issue: Issue 1
  // Joseph latest magazine issue: Issue 1
  magazine.unsubscribe(alan);

  magazine.setCurrentIssue('Issue 2');
  // Joseph latest magazine issue: Issue 2
};
